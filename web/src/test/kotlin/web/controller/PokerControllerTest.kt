...
class PokerControllerTest {
    ...
    @Test
    fun `lobby page wires jackpotPool jackpotWinPct and jackpotStakeAnchor model attributes so the banner renders`() {
        ...
        // the admin-set probability instead of a hardcoded 1%, and
        // `jackpotStakeAnchor` so the stake-scaling threshold quoted in
        // the banner matches the per-guild `JACKPOT_STAKE_ANCHOR` config.
        ...
        every { jackpotService.stakeAnchor(guildId) } returns 750L
        ...
        assertEquals(750L, model.getAttribute("jackpotStakeAnchor"))
    }

    @Test
    fun `tablePage wires jackpotPool jackpotWinPct and jackpotStakeAnchor model attributes so the banner renders`() {
        ...
        every { jackpotService.stakeAnchor(guildId) } returns 250L
        ...
        assertEquals(250L, model.getAttribute("jackpotStakeAnchor"))
    }
