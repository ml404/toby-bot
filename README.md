...
- **Intro songs, everywhere:** Up to three per user per server, from a link or an MP3, each with its own volume and optional clip bounds. Each can be switched out of the join rotation without being deleted. Discord (`/setintro`, `/listintros`, `/editintro`, `/deleteintro`, and a right-click **Set as my intro** message action) and the web dashboard write the same rows and share one set of rules (`common.intro.IntroClip` / `IntroSlots`), so the 15-second cap, the `mm:ss` grammar and the validation messages can't drift between surfaces.
