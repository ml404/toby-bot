...
class ExcuseCommand(private val excuseService: IExcuseService) : IMiscCommand {
    ...
    private fun approveExcuse(requestingUserDto: UserDto?, event: SlashCommandInteractionEvent, deleteDelay: Int?) {
    ...
    private fun deleteExcuse(requestingUserDto: UserDto?, event: SlashCommandInteractionEvent, deleteDelay: Int?) {
        if (requestingUserDto?.superUser == true) {
            val excuseId = event.getOption(EXCUSE_ID)?.asLong ?: return
