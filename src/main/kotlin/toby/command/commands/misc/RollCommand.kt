...
class RollCommand : IMiscCommand {
    private val DICE_NUMBER = "number"
    private val DICE_TO_ROLL = "amount"
    private val MODIFIER = "modifier"
    override fun handle(ctx: CommandContext, requestingUserDto: UserDto, deleteDelay: Int?) {
        val event = ctx.event
        val diceValueOptional = Optional.ofNullable(event.getOption(DICE_NUMBER)).map { obj: OptionMapping -> obj.asInt }
        val diceToRollOptional = Optional.ofNullable(event.getOption(DICE_TO_ROLL)).map { obj: OptionMapping -> obj.asInt }
        val diceModifierOptional = Optional.ofNullable(event.getOption(MODIFIER)).map { obj: OptionMapping -> obj.asInt }
    ...
    fun handleDiceRoll(event: IReplyCallback, diceValue: Int, diceToRoll: Int, modifier: Int): WebhookMessageCreateAction<Message> {
        event.deferReply().queue()
        val sb = buildStringForDiceRoll(diceValue, diceToRoll, modifier)
        val embedBuilder = EmbedBuilder()
                .addField(MessageEmbed.Field(String.format("%dd%d + %d", diceToRoll, diceValue, modifier), sb.toString(), true))
    ...
    override val optionData: List<OptionData>
        get() {
            val diceNumberOption = OptionData(OptionType.INTEGER, DICE_NUMBER, "What sided dice would you like to roll?")
