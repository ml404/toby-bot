...
class PollCommand : IModerationCommand {
    override fun handle(ctx: CommandContext, requestingUserDto: UserDto, deleteDelay: Int?) {
        ...
        if (choiceOptional.isPresent) {
            ...
            if (pollArgs.size > 10) {
                hook.sendMessageFormat("Please keep the poll size under 10 items, or else %s.", event.guild!!.jda.getEmojiById(
                    Emotes.TOBY)).queue(invokeDeleteOnMessageResponse(deleteDelay!!))
                return
            }
