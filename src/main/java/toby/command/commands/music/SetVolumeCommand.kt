...
class SetVolumeCommand : IMusicCommand {
    ...
    private fun setNewVolume(event: SlashCommandInteractionEvent, instance: PlayerManager, member: Member?, requestingUserDto: UserDto?, deleteDelay: Int?) {
    ...
    fun sendErrorMessage(event: SlashCommandInteractionEvent, deleteDelay: Int?) {
        event.hook.sendMessageFormat("You aren't allowed to change the volume kid %s", event.guild!!.jda.getEmojiById(Emotes.TOBY)).setEphemeral(true).queue(invokeDeleteOnMessageResponse(deleteDelay!!))
    }
