...
class SocialCreditCommand(private val userService: IUserService) : IModerationCommand {
    ...
    private fun calculateAndUpdateSocialCredit(ctx: CommandContext?, event: SlashCommandInteractionEvent, requestingUserDto: UserDto?, requestingMember: Member?, deleteDelay: Int?) {
    ...
    private fun createAndPrintLeaderboard(event: SlashCommandInteractionEvent, deleteDelay: Int?) {
        ...
        sortedMap.forEach { (discordId, score) ->
            ...
            val member = guild.getMemberById(discordId) ?: return@forEach
