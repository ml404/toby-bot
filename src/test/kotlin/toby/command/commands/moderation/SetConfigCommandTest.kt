...
import toby.jpa.dto.ConfigDto.Configurations.DELETE_DELAY
import toby.jpa.dto.ConfigDto.Configurations.VOLUME
...
internal class SetConfigCommandTest : CommandTest {
    ...
    fun testSetConfig_notAsServerOwner_sendsErrorMessage() {
        ...
        Mockito.`when`(configService.getConfigByName(VOLUME.name, "1")).thenReturn(null)

        //Act
        setConfigCommand.handle(commandContext, CommandTest.requestingUserDto, 0)

        //Assert
        Mockito.verify(configService, Mockito.times(0)).getConfigByName(VOLUME.configValue, "1")
        Mockito.verify(configService, Mockito.times(0)).createNewConfig(any())
    ...
    fun testSetConfig_withOneConfig_createsThatConfig() {
        ...
        Mockito.`when`<List<OptionMapping>>(CommandTest.event.options)
            .thenReturn(listOf(volumeOptionMapping))
        Mockito.`when`(configService.getConfigByName(VOLUME.configValue, "1"))
            .thenReturn(null)
        ...
        Mockito.verify(configService, Mockito.times(1))
            .getConfigByName(VOLUME.configValue, "1")
        Mockito.verify(configService, Mockito.times(1)).createNewConfig(any())
        Mockito.verify(CommandTest.interactionHook, Mockito.times(1))
    ...
    fun testSetConfig_withDeleteDelay_createsThatConfig() {
        ...
        Mockito.`when`<OptionMapping>(
            CommandTest.event.getOption(
                DELETE_DELAY.name.lowercase(
                    Locale.getDefault()
                )
            )
        ).thenReturn(deleteDelayOptionMapping)
        Mockito.`when`(deleteDelayOptionMapping.asInt).thenReturn(20)
        Mockito.`when`(deleteDelayOptionMapping.name).thenReturn(DELETE_DELAY.name)
        Mockito.`when`(CommandTest.member.isOwner).thenReturn(true)
        Mockito.`when`<List<OptionMapping>>(CommandTest.event.options)
            .thenReturn(listOf(deleteDelayOptionMapping))
        Mockito.`when`(configService.getConfigByName(DELETE_DELAY.configValue, "1")).thenReturn(null)

        //Act
        setConfigCommand.handle(commandContext, CommandTest.requestingUserDto, 0)

        //Assert
        Mockito.verify(configService, Mockito.times(1)).getConfigByName(DELETE_DELAY.configValue, "1")
        Mockito.verify(configService, Mockito.times(1)).createNewConfig(any())
    ...
    fun testSetConfig_withMultipleSpecified_createsTwoConfig() {
        ...
        Mockito.`when`(configService.getConfigByName(DELETE_DELAY.name, "1"))
            .thenReturn(null)
        Mockito.`when`(configService.getConfigByName(VOLUME.name, "1")).thenReturn(null)
        ...
        //Assert
        Mockito.verify(configService, Mockito.times(1)).getConfigByName(DELETE_DELAY.configValue, "1")
        Mockito.verify(configService, Mockito.times(1)).getConfigByName(VOLUME.configValue, "1")
        Mockito.verify(configService, Mockito.times(2)).createNewConfig(any())
        Mockito.verify(CommandTest.interactionHook, Mockito.times(1))
            .sendMessageFormat(ArgumentMatchers.eq("Set default volume to '%s'"), eq(20))
        Mockito.verify(CommandTest.interactionHook, Mockito.times(1)).sendMessageFormat(
            eq("Set default delete message delay for TobyBot music messages to '%d' seconds"),
            eq(20)
        )
    }
    ...
    fun testSetConfig_withMoveChannel_createsThatConfig() {
        ...
        Mockito.verify(configService, Mockito.times(1)).createNewConfig(any())
        Mockito.verify(CommandTest.interactionHook, Mockito.times(1)).sendMessageFormat(
            eq("Set default move channel to '%s'"),
            eq("Channel Name")
        )
    }
}