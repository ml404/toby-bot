...
interface MusicCommandTest : CommandTest {
    fun setupCommonMusicMocks() {
        ...
        Mockito.`when`(musicManager.audioPlayer).thenReturn(audioPlayer)
        Mockito.`when`(musicManager.sendHandler).thenReturn(audioPlayerSendHandler)
        Mockito.`when`(musicManager.scheduler).thenReturn(trackScheduler)
        ...
        Mockito.`when`(trackScheduler.queue).thenReturn(ArrayBlockingQueue(1))
